...
public class InCallProximityManager {
  interface ProximitySensorAware {
    void onProximitySensorChangedState(boolean near);
  }
  ...
  /** Construction */
  static InCallProximityManager create(Context context, final ProximitySensorAware listener) {
    return new InCallProximityManager(context, listener);
  }

  private InCallProximityManager(Context context, final ProximitySensorAware listener) {
      ...
      proximitySensor =
          AppRTCProximitySensor.create(
              ...
              () -> {
                ...
                listener.onProximitySensorChangedState(proximitySensor.sensorReportsNearState());
              });
    }
  }
  ...
  private void checkProximitySupport(Context context) {
    ...
    try {
      ...
      if (android.os.Build.VERSION.SDK_INT < 17) {
        ...
        // --- android 4.2+
        Method method =
            powerManager.getClass().getDeclaredMethod("isWakeLockLevelSupported", int.class);
